defmodule GoParser.Node do
  alias __MODULE__
  alias GoParser.Util.Tools
  require Logger
  ...
  def get_move(node) do
    # Do not use double quote because of charlist !!!
    ...
    cond do
      ...
      !! move_black ->
        coordinate = Tools.move_to_coordinates(move_black) |> List.first
        {:add_move, {coordinate, :black}}
      !! move_white ->
        coordinate = Tools.move_to_coordinates(move_white) |> List.first
  ...
  def get_placements(node) do
    ...
    cond do
      !! place_black && !! place_white ->
        Logger.debug("node #{node.id} : simultaneous placements for B/W.")
